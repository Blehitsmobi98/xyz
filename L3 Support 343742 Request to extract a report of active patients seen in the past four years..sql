...
WITH my_cte AS ( select distinct PatientRaces.iPatID, (SELECT ','+ rc.vrace FROM 
     ...
     my_cte2 AS ( select vaccno,
                SUBSTRING(
                       (
                       select (', '+ rtrim(vethnicity)) from patientethnicity where vaccno = t.vaccno for XML Path ('') )
                     ,2,1000) as 'Ethnicities' from PatientEthnicity t group by t.vaccno),
...
my_cte2.ethnicities [Patient Ethnicity],
...
left join
          my_cte2 on pat.vaccno = my_cte2.vaccno
...
left join
           (select pat2.IPATID,pat2.IREFPRVID, prv.VSSN from PMPTXFT pat2 inner join PMPRVFT prv on pat2.IREFPRVID=prv.IPRVID where pat2.IPATID= 51928  ) as refprov on pat.IPATID=refprov.IPATID 
